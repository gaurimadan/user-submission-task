...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        const fetchSubmissions = async () => {
            try {
                const response = await axios.get('https://user-submission-task-production.up.railway.app/api/submissions');
                                        ...
                                        <ImageList sx={{ width: '100%', height: 'auto', maxHeight: 400 }} cols={3} rowHeight={164}>
                                            {submission.images.map((image, index) => (
                                                <ImageListItem key={index}>
                                                    <img
                                                        src={`https://user-submission-task-production.up.railway.app/${image}`}
